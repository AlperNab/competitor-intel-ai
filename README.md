...
**Domain:** `Business / Competitive Intelligence` · **Suite:** `General Automation Suite` · **Accent:** `#93c5fd`
...
**Competitor Intel Ai** is a standalone, browser-based AI workflow app for **Business / Competitive Intelligence**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.

**Core job:** Competitor URL → sales/marketing intelligence.
...
|---|---:|:---:|---|---|
| `competitor_url`<br>Competitor URL | text | Yes | — | Affects input: Competitor URL. |
...
| `your_product_profile`<br>your product profile | select | No | — | Affects customization: your product profile. |
| `icp`<br>ICP | text | No | — | Affects customization: ICP. |
| `region`<br>region | text | No | — | Affects customization: region. |
| `competitor_list`<br>competitor list | textarea | No | — | Affects customization: competitor list. |
| `sales_angle`<br>sales angle | text | No | — | Affects customization: sales angle. |
| `pricing_model`<br>pricing model | select | No | — | Affects customization: pricing model. |
| `monitoring_frequency`<br>monitoring frequency | text | No | — | Affects customization: monitoring frequency. |
...
uvicorn app.main:app --host 127.0.0.1 --port 9110
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9110`. |
...
- Export download
- Job history
- HTTP fetch/Browser/SerpAPI optional
...
```bash
curl http://127.0.0.1:9110/api/health
```
...
```bash
curl -X POST http://127.0.0.1:9110/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9110 uvicorn app.main:app --host 0.0.0.0 --port 9110
```
...
- `plugins/competitor-intel-ai.json` for plugin metadata.
